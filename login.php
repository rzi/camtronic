	...
	if($result = mysqli_query($link,"select * from spl_users where sesa = '".htmlspecialchars($sesa)."' AND password = '".htmlspecialchars($password)."'")){
       $row_cnt = mysqli_num_rows($result);
        ...
        if ($row_cnt >0) {   
            ...
            if ($row['sesa']=="1"){
                header( 'refresh: 0; url=http://localhost/users_interface.php' ) ;
            } else{
                if( $nazwa=="Trener"){
                    header( 'refresh: 0; url=http://localhost/trainer_menu.php' ) ;
                }else{
                    header( 'refresh: 0; url=http://localhost/spl.php' ) ; 
                }
